...
from predict import predire_arrosage, _charger_artefacts
from dataset import CATEGORIES_SOL, CATEGORIES_CULTURE, entree_unique
...
# Les 4 zones de la ferme simulée (sol/culture fixes, humidité modifiable)
ZONES = [
    {"nom": "Zone A", "sol": "Sableux", "culture": "Maraîchage", "humidite_defaut": 22},
    {"nom": "Zone B", "sol": "Limoneux", "culture": "Céréales", "humidite_defaut": 55},
    {"nom": "Zone C", "sol": "Argileux", "culture": "Arboriculture", "humidite_defaut": 68},
    {"nom": "Zone D", "sol": "Sableux", "culture": "Céréales", "humidite_defaut": 15},
]

# État du réservoir conservé en mémoire côté serveur (une seule ferme simulée
# pour la démo — pas de base de données, redémarre à 60% si le serveur redémarre).
...
@app.get("/api/config")
def config():
    return {"sols": CATEGORIES_SOL, "cultures": CATEGORIES_CULTURE, "zones": ZONES}


@app.post("/api/predire")
def predire(req: PredictionRequest):
    message = predire_arrosage(
        req.humidite_sol, req.temperature_air, req.humidite_air,
...
def analyser_zones(req: ZonesRequest):
    ...
    if req.pluie_prevue_mm > 0:
        reservoir_global, msg_pluie = recharger_pluie(reservoir_global, req.pluie_prevue_mm)
    else:
    ...
    for zone, humidite_sol in zip(ZONES, req.humidites_sol):
        X = entree_unique(humidite_sol, req.temperature_air, req.humidite_air,
                           req.pluie_prevue_mm, zone["sol"], zone["culture"])
        ...
        resultats.append({
            "nom": zone["nom"], "sol": zone["sol"], "culture": zone["culture"],
            "humidite_sol": humidite_sol, "statut": statut, "confiance": proba,
...
def recharger(req: RechargeRequest):
    ...
    return {"niveau_l": reservoir_global.niveau_l, "niveau_pct": reservoir_global.niveau_pct}
